...
function analyzeOrderData(orders) {
    ...
    orders.forEach((order, index) => {
        ...
        console.log(`   狀態: ${order.status || 'undefined'}`);
        console.log(`   總金額: NT$ ${order.totalAmount || 'undefined'}`);
        console.log(`   訂單類型: ${order.orderType || 'undefined'}`);
        console.log(`   桌號: ${order.tableNumber || 'N/A'}`);
        console.log(`   區域: ${order.area || 'N/A'}`);
        console.log(`   創建時間: ${order.createdAt || 'undefined'}`);
        
        console.log(`\n   商品項目 (${order.items ? order.items.length : 0} 項):`);
